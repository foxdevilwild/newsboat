...
void utils::set_common_curl_options(CURL* handle, configcontainer* cfg)
{
	...
	if (proxyauth != "") {
		...
		curl_easy_setopt(
			handle, CURLOPT_PROXYUSERPWD, proxyauth.c_str());
	}
	if (proxytype != "") {
		LOG(level::DEBUG,
			"utils::set_common_curl_options: proxytype = %s",
			proxytype);
		curl_easy_setopt(
			handle, CURLOPT_PROXYTYPE, get_proxy_type(proxytype));
	}
	...
	if (cookie_cache != "") {
		curl_easy_setopt(
			handle, CURLOPT_COOKIEFILE, cookie_cache.c_str());
		curl_easy_setopt(
			handle, CURLOPT_COOKIEJAR, cookie_cache.c_str());
	}
